...
```{r structured-lgc, echo=T, eval=F, attr.source='.numberLines', ref = "structured-model", codecap = "OpenMx Code for Structured Latent Growth Curve Model"}
#Manifest variable names (i.e., names of columns cotaining data points at each time point [e.g., ])
...
#initial checks 
  ...
  manifest_vars <- extract_manifest_var_names(data_wide = data_wide)
  latent_vars <- c('theta', 'alpha', 'beta', 'gamma')
...
model <- mxModel(
  ...
  #Residual variances; by using one label, they are assumed to all be equal (homogeneity of variance)
  mxPath(from = manifest_vars,arrows=2, free=TRUE,  labels='epsilon', values = 1, lbound = 0),

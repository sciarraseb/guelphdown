...
#cond_summary_exp_3 <- compute_condition_summary(param_summary_data = combined_analytical_exp_3_def, facet_var = #'time_structuredness', 
#                          ind_vars = c('number_measurements', 'sample_size', 'time_structuredness'))
#
...
In examining the effects of time structuredness, the results show that modelling accuracy decreases as time structuredness decreases. Importantly, the decrease in modelling accuracy is often constant regardless of the number of measurements or sample size. Because data are likely to be time unstructured, then the constant error incurred seems inevitable and this can be disconcerting. Fortunately, this error can be avoided by using actual times of measurements, otherwise known as definition variables [@mehta2000; @mehta2015]. Figure \ref{fig:latent-def} shows a latent variable with definition variables. 

To show that definition variables can eliminate the error incurred by time-unstructured data, I ran an additional set of simulations.   

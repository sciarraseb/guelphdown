...
Given that each simulation experiment manipulated no more than three independent variables so that results could be readily interpreted [@halford2005], other variables had to be set to constant values. In Experiment 2, two important variables were set to constant values: nature of change and time structuredness. For nature of change, I set the value for the fixed-effect days-to-halfway elevation parameter ($\upbeta_{fixed}$) across all cells to have a value of 180. For time structuredness, data across all cells were generated to be time structured. 
